...
3. `checker_positive` 是 AgentDojo 官方 security checker 输出；它**不等于**经工具调用和环境变化确认的外部伤害。后者见 `triage/`。
4. Recovery 原始结果中的某些路径字段记录了实验机器上的历史位置；在本仓库中查找对应快照请使用 `run-index.json` 的相对路径。请根据环境快照、工具轨迹和 evaluator 输出解释结果，不以 final response 判定修复成功。
5. 这些文件包含原始 benchmark 内容和攻击 payload。两份研究原文另存于 `docs/source/`，同时完整收录在根目录 README。
